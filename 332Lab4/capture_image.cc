...
int main(void) {
    ...
    unsigned char cropped_bw[240 * 240];
    unsigned char pixels_scaled[28 * 28];
    ...
    *(video_in_dma + 3) = 0x4;
        ...
        if (btn_val < 7 && btn_val > 0) {
            ...
            for (int y = 0; y < IMAGE_HEIGHT; y++) {
                for (int x = 0; x < IMAGE_WIDTH; x++) {
                    ...
                    pixels_bw[index] = (unsigned char)(video_mem[(y << 9) + x] & 0xFF); // crude grayscale
                }
            }

            // Crop 240x240 from center of 320x240
            //int crop_offset_x = (IMAGE_WIDTH - 240) / 2;
            for (int y = 0; y < 240; y++) {
                for (int x = 0; x < 240; x++) {
                    cropped_bw[y*240 + x] = pixels_bw[y*IMAGE_WIDTH + x];
                }
            }
            

            // Scale cropped image to 28x28
            scaleImagePreservingAspectRatio(cropped_bw, pixels_scaled, 240, 240, 28, 28);

            // Save the scaled grayscale image (28x28)
            saveImageGrayscale("final_image_bw.bmp", pixels_bw, 320, 240);
            saveImageGrayscale("final_image_bw_cropped.bmp", cropped_bw, 240, 240);
            saveImageGrayscale("final_image_bw_downsampled.bmp", pixels_scaled, 28, 28);
    ...
    // Cleanup
    munmap(virtual_base2, IMAGE_SPAN);
    munmap(virtual_base, HW_REGS_SPAN);
    close(fd);
    return 0;
}
